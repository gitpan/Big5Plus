...
######################################################################
#
# Big5Plus - "Yet Another JPerl" Source code filter to escape Big5Plus
#
...
if ((not Ebig5plus::e("$filename.e")) or ($e_mtime < $mtime) or ($mtime < $__mtime__)) {
    my $fh = Symbol::gensym();
    sysopen($fh, "$filename.e", O_WRONLY | O_TRUNC | O_CREAT) or croak "$__FILE__: Can't open file: $filename.e";
    print {$fh} Big5Plus::escape_script($filename);
...
sub Big5Plus::escape_script {
    ...
    if (m/^ use Ebig5plus(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf(<<'END', $Ebig5plus::VERSION); # require run-time routines version
use Ebig5plus %s;
use re 'eval';
END
    ...
    for my $widget (qw(
        ...
        if (m/ $widget /xms) {
            $tkmodule .= "        eval qq{ use Big5Plus::Tk::$widget; };\n";
        }
    }

    my $use_tk = <<"USE_TK";
BEGIN {
    eval qq{ use Big5Plus::Encode; };
    if (\$] >= 5.007) {
$tkmodule
    }
    else {
        eval qq{ use Big5Plus::Tk::Entry55; };
        eval qq{ use Big5Plus::Tk::MainWindow; };
    }
}
USE_TK
    if (s/^ (\s* use \s+ Tk [^:;]* ; \s*? \n) /$1$use_tk/oxmsg) {
        s/ \b (MainWindow \s* -> \s* new) \b /Big5Plus::Tk::$1/oxmsg;
        s/^ (\s* use \s+ )(Tk::(?:Ballon|BrowseEntry|ColorEditor|Dialog|DialogBox|DirTree|FileSelect|HList|ROText|Table|TixGrid|TList|Tree) [^:;]* ; \s*? ) \n /BEGIN { eval qq{ $1$2 ${1}Big5Plus::${2} }};\n/oxmsg;
        s/^ (\s* use \s+ )(Tk::(?:LabFrame) [^:;]*) ; \s*? \n /BEGIN { eval qq{ $1$2; ${1}Big5Plus::${2}55; }};\n/oxmsg;
    }
...
# functions of package Ebig5plus
    ...
    elsif (m{\G \b chdir \b   (?! \s* => )                    }oxgc) { $slash = 'm//'; return   'Ebig5plus::chdir';              }
...
# functions of package Ebig5plus
        ...
        elsif ($string =~ m{\G \b chdir \b                                   }oxgc) { $e_string .=   'Ebig5plus::chdir';             $slash = 'm//'; }
...
sub e_qr {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', $ope, $delimiter, "$your_gap(?:", @char, '>]}' x ($left_e - $right_e), ')(?{Ebig5plus::m_matched})', $end_delimiter, $modifier;
    }
    return     join '', $ope, $delimiter, "$your_gap(?:", @char,                               ')(?{Ebig5plus::m_matched})', $end_delimiter, $modifier;
}
...
sub e_s1 {
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', $ope, $delimiter, "($your_gap)(?:", @char, '>]}' x ($left_e - $right_e), ')(?{Ebig5plus::s_matched})', $end_delimiter, $modifier;
    }
    return     join '', $ope, $delimiter, "($your_gap)(?:", @char,                               ')(?{Ebig5plus::s_matched})', $end_delimiter, $modifier;
}
...
sub e_use_noparam {
    ...
    for my $prefix (@INC) {
        ...
        if (sysopen($fh, $realfilename, O_RDONLY)) {
            ...
            if ($script =~ m/^ \s* use \s+ Big5Plus \s* ([^;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Ebig5plus::require '$expr'; $module->import(); }];
            }
            last;
        }
    }
...
sub e_use {
    ...
    for my $prefix (@INC) {
        ...
        if (sysopen($fh, $realfilename, O_RDONLY)) {
            ...
            if ($script =~ m/^ \s* use \s+ Big5Plus \s* ([^;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Ebig5plus::require '$expr'; $module->import($list); }];
            }
            last;
        }
    }
...
=item * chop --> Ebig5plus::chop

=item * split --> Ebig5plus::split

=item * length

=item * substr

=item * index --> Ebig5plus::index

=item * rindex --> Ebig5plus::rindex

=item * pos

=item * lc --> Ebig5plus::lc or Ebig5plus::lc_

=item * uc --> Ebig5plus::uc or Ebig5plus::uc_

=item * ord (when no import)

=item * reverse (when no import)

=item * tr/// or y/// --> Ebig5plus::tr

/b and /B modifier can also be used.

=item * chdir --> Ebig5plus::chdir

support chr(0x5C) ended path on only perl5.005 if MSWin32.

=item * do --> Ebig5plus::do

=item * require --> Ebig5plus::require

=item * use Perl::Module @list; --> BEGIN { Ebig5plus::require 'Perl/Module.pm'; Perl::Module->import(@list); }

=item * use Perl::Module (); --> BEGIN { Ebig5plus::require 'Perl/Module.pm'; }
...
=item * chr --> Ebig5plus::chr or Ebig5plus::chr_

multiple octet code can also be handled.

=item * -X --> Ebig5plus::X or Ebig5plus::X_

support chr(0x5C) ended path on MSWin32.

=item * glob --> Ebig5plus::glob or Ebig5plus::glob_

  @glob = Ebig5plus::glob($string);
  @glob = Ebig5plus::glob_;

Ebig5plus::glob provides a portable enhanced DOS-like globbing for the Big5Plus software.
Ebig5plus::glob lets you use wildcards in directory paths, is case-insensitive, and
...
=item * lstat --> Ebig5plus::lstat or Ebig5plus::lstat_

support chr(0x5C) ended path on MSWin32.

=item * opendir --> Ebig5plus::opendir

support chr(0x5C) ended path on MSWin32.

=item * stat --> Ebig5plus::stat or Ebig5plus::stat_

support chr(0x5C) ended path on MSWin32.

=item * unlink --> Ebig5plus::unlink

support chr(0x5C) ended path on MSWin32.

=item * ord --> Big5Plus::ord or Big5Plus::ord_

multiple octet code can also be handled when "use Big5Plus qw(ord);".
It means not compatible with JPerl.

=item * reverse --> Big5Plus::reverse

multiple octet code can also be handled in scalar context when "use Big5Plus qw(reverse);".
It means not compatible with JPerl.
